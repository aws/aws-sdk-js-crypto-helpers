...
describe('Sha256', () => {
  ...
  it('should trap processing errors for hmacs', async () => {
    ...
    (importKey as sinon.SinonStub).resolves(Uint8Array.from([0xde, 0xad, 0xbe, 0xef]));
  ...
  it(
    ...
    async () => {
      ...
      const {digest} = locateWindow().crypto.subtle;
      (digest as sinon.SinonStub).resolves(Uint8Array.from([0xde, 0xad, 0xbe, 0xef]));

      sha256.update(new ArrayBuffer(4));

      expect(await sha256.digest())
        .to.deep.equal(Uint8Array.from([0xde, 0xad, 0xbe, 0xef]));
    }
  );
  ...
  it(
    ...
    async () => {
      const {importKey, sign} = locateWindow().crypto.subtle;
      (importKey as sinon.SinonStub).resolves(Uint8Array.from([0xde, 0xad, 0xbe, 0xef]));
      (sign as sinon.SinonStub).resolves(Uint8Array.from([0xde, 0xad, 0xbe, 0xef]));
