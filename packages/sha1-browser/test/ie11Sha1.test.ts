...
import "mocha";
import { Sha1 } from "../src/ie11Sha1";
import { MsWindow } from "@aws-crypto/ie11-detection";
import { SHA_1_HMAC_ALGO } from "../src/constants";
...
describe("Sha1", () => {
  ...
  it("should create a hash object by default", () => {
    ...
    expect((<sinon.SinonStub>digest).firstCall.args).to.deep.equal(["SHA-1"]);
  });
  ...
  it("should import ArrayBuffer secrets", async () => {
    ...
    const sha1 = new Sha1(secret.buffer);
    await flushPromises();
