...
import "mocha";
import { isMsWindow, MsWindow } from "../src/MsWindow";
...
describe("isMsWindow", () => {
  it("should return false if an object does not fulfill the MsWindow interface", () => {
    expect(isMsWindow({} as any)).to.eql(false);
  });

  it("should return true if an object fulfills the MsWindow interface", () => {
    expect(isMsWindow(fakeMsWindow)).to.eql(true);
  });
});
